...
describe('TextHighlighter Integration Tests', () => {
  ...
  describe('Complete workflow integration', () => {
    it('should handle complete highlight creation workflow', () => {
      ...
      const result1 = addHighlight(highlights, 1, 2);
      expect(result1.highlights).toHaveLength(1);
      expect(result1.newHighlight.start).toBe(1);
      expect(result1.newHighlight.end).toBe(2);
      expect(result1.newHighlight.colorId).toBe(1);
      
      // 2. Create second highlight with next color
      const result2 = addHighlight(result1.highlights, 4, 5);
      expect(result2.highlights).toHaveLength(2);
      expect(result2.newHighlight.colorId).toBe(2); // Next color
    ...
    it('should handle highlight modification workflow', () => {
      let highlights = [
        { id: 'h1', start: 1, end: 3, colorId: 1 },
        { id: 'h2', start: 5, end: 6, colorId: 2 }
      ];
    ...
    it('should handle color ID management workflow', () => {
      ...
      expect(getColorFromId(2)).toBe('var(--highlight-2)');
      expect(getColorFromId(20)).toBe('var(--highlight-20)');
      
      // 3. Test cycling behavior - call enough times to see wrap around
      const results = [];
      for (let i = 0; i < 21; i++) {
  ...
  describe('Error handling and boundary conditions', () => {
    ...
    it('should handle invalid indices', () => {
      const highlights = [{ id: 'h1', start: 1, end: 3, colorId: 3 }];
    ...
    it('should handle overlapping highlights correctly', () => {
      const highlights = [
        { id: 'h1', start: 0, end: 3, colorId: 3 },
        { id: 'h2', start: 2, end: 5, colorId: 7 },
        { id: 'h3', start: 6, end: 8, colorId: 11 }
      ];
  ...
  describe('Highlight expansion functionality', () => {
    it('should expand highlight to the right correctly', () => {
      ...
      const originalHighlight = { id: 'h1', start: 1, end: 2, colorId: 1 };
    ...
    it('should expand highlight to the left correctly', () => {
      ...
      const originalHighlight = { id: 'h1', start: 2, end: 3, colorId: 1 };
    ...
    it('should expand highlight in both directions correctly', () => {
      ...
      const originalHighlight = { id: 'h1', start: 2, end: 2, colorId: 1 };
    ...
    it('should prevent expansion when it would cause overlap', () => {
      const highlights = [
        { id: 'h1', start: 2, end: 3, colorId: 1 },
        { id: 'h2', start: 5, end: 6, colorId: 1 }
      ];
