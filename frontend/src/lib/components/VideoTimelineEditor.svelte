<script>
  ...
  let draggedStart = $state(0); // Temporary start time during dragging
  let draggedEnd = $state(0); // Temporary end time during dragging
  ...
  let endIndex = $derived(() => {
    ...
    return Math.min(
      ...
      startIndex + visibleCount + visibleBuffer * 2
    );
  });
  ...
  function findCurrentWordIndex(time, words) {
    ...
    let exactIndex = words.findIndex(
  ...
  async function loadTranscriptionWords() {
    ...
    try {
      ...
      console.log(
        "Loading transcription words for project:",
        projectId,
        "videoClipId:",
        highlight.videoClipId
      );
      ...
      if (
        clip &&
        clip.transcriptionWords &&
        clip.transcriptionWords.length > 0
      ) {
        transcriptionWords = clip.transcriptionWords;
        console.log(
          "Loaded transcription words:",
          transcriptionWords.length,
          transcriptionWords.slice(0, 5)
        );
      } else {
        transcriptionWords = [];
  ...
  function handleGlobalMouseMove(event) {
    ...
    // Update temporary values during drag, not the actual state
    if (dragMarkerType === "start") {
      draggedStart = Math.max(0, Math.min(newTime, draggedEnd - 0.1));
    } else if (dragMarkerType === "end") {
      draggedEnd = Math.min(duration, Math.max(newTime, draggedStart + 0.1));
    }
  }

  function handleGlobalMouseUp() {
    if (isDraggingMarker) {
      // Apply the dragged values to the actual state only on mouse up
  ...
  function scrollToWordIndex(wordIndex) {
    if (wordIndex < 0 || !wordsContainer || transcriptionWords.length === 0)
      return;

    autoScrolling = true;
    // Center the word in the container
    const targetScrollTop =
      wordIndex * wordHeight - containerHeight / 2 + wordHeight / 2;
    const maxScrollTop =
      transcriptionWords.length * wordHeight - containerHeight;
...
<div class="video-timeline-editor flex flex-col h-full">
  <!-- Timeline Controls -->
  <div
    class="timeline-controls flex items-center justify-between p-3 border-b bg-background"
  >
  ...
  <div class="border rounded-lg">
    <div class="p-4">
      ...
      >
        ...
        <!-- Highlight region -->
        {#if duration > 0}
          {@const displayStart = isDraggingMarker ? draggedStart : editedStart}
          {@const displayEnd = isDraggingMarker ? draggedEnd : editedEnd}
          <div
            class="absolute top-0 h-full bg-blue-500/30"
            style="left: {(displayStart / duration) *
              100}%; width: {((displayEnd - displayStart) / duration) * 100}%"
          ></div>
        {/if}
        ...
        {#if duration > 0}
          ...
          <div
            ...
            style="left: calc({(displayStart / duration) * 100}% - 4px)"
        ...
        {#if duration > 0}
          ...
          <div
            ...
            style="left: calc({(displayEnd / duration) * 100}% - 4px)"
      ...
      <!-- Words display -->
      <div
        class="border rounded bg-background/50"
        ...
        {:else if transcriptionWords.length === 0}
          ...
          >
            <div class="text-center">
              ...
              <p class="text-xs">
                Transcribe the video to see word-level timing
              </p>
            </div>
          </div>
        {:else}
          ...
          >
            <div class="p-2">
              {#each transcriptionWords as word, wordIndex}
                {@const isInHighlight =
                  word.start < editedEnd && word.end > editedStart}
                {@const isCurrentWord =
                  currentWordIndex >= 0 && wordIndex === currentWordIndex}

                <div
                  class="flex items-center gap-2 p-1 rounded text-xs transition-colors
                         {isCurrentWord
                    ? 'bg-orange-500/30 border-l-4 border-orange-500 font-bold shadow-sm'
                    : isInHighlight
