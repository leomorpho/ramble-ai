...
// QueryVideoClips chains the current query on the "video_clips" edge.
func (pq *ProjectQuery) QueryVideoClips() *VideoClipQuery {
	query := (&VideoClipClient{config: pq.config}).Query()
...
// WithVideoClips tells the query-builder to eager-load the nodes that are connected to
// the "video_clips" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithVideoClips(opts ...func(*VideoClipQuery)) *ProjectQuery {
	query := (&VideoClipClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withVideoClips; query != nil {
...
func (pq *ProjectQuery) loadVideoClips(ctx context.Context, query *VideoClipQuery, nodes []*Project, init func(*Project), assign func(*Project, *VideoClip)) error {
	...
	query.withFKs = true
	query.Where(predicate.VideoClip(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.VideoClipsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_video_clips
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_video_clips" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
